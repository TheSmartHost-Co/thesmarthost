...
/**
 * Focused modal for editing only the time-sheet wage fields:
 * hourly rate, weekly max hours, currency. Avoids the full
 * UpdateTeamMemberModal's name/phone/permissions surface so the PM can
 * adjust pay settings in one click from the Team Time Sheet page.
 *
 * Saves via the existing PUT /api/team-members/:id. The backend uses
 * COALESCE on wage fields, so omitting any field leaves it unchanged.
 */
const WageSettingsModal: React.FC<WageSettingsModalProps> = ({
  ...
  // We fetch the full team-member on open so we can echo back name / phone /
  // status / permissions on save. The existing PUT /api/team-members/:id is a
  // full-row endpoint (omitted fields get cleared/defaulted), so we have to
  // send everything to safely change just the wage fields.
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await getTeamMember(member.teamMemberId)
        if (cancelled) return
        if (res.status === 'success') {
          setFullMember(res.data)
        } else {
          showNotification(res.message || 'Could not load team member.', 'error')
        }
      } catch (err: any) {
        if (!cancelled) showNotification(err?.message || 'Could not load team member.', 'error')
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      // Send the full row so the existing endpoint doesn't clear other fields.
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-1">
            Leave blank to keep unset. Future entries snapshot this rate; existing entries keep their original snapshot.
          </p>
        </div>
