...
export default function CloneWalkthroughTemplateModal({
  ...
  const handleClone = async () => {
    ...
    try {
      ...
      showNotification(
        err instanceof Error ? err.message : 'Error cloning template',
        'error'
      )
    } finally {
      setCloning(false)
      ...
      >
        <div className="px-6 py-5 border-b border-gray-100 flex items-start justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="text-base font-bold text-gray-900">Clone Walkthrough Template</h2>
              <p className="text-xs text-gray-500 mt-0.5">
                Duplicates groups and items. Property assignments are not copied.
              </p>
            </div>
          </div>
