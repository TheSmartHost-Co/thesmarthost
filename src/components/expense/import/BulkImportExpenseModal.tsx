...
import AssignPropertiesStep, { PropertyAssignment } from './steps/AssignPropertiesStep'
...
interface BulkImportExpenseModalProps {
  isOpen: boolean
  onClose: () => void
  onImportComplete: (importedExpenses: Expense[]) => void
  properties: Property[]
}
...
const STEPS: { key: Step; label: string; icon: React.ElementType }[] = [
  ...
  { key: 'assign', label: 'Assign Properties', icon: HomeIcon },
  { key: 'preview', label: 'Preview & Import', icon: EyeIcon }
]

const BulkImportExpenseModal: React.FC<BulkImportExpenseModalProps> = ({
  isOpen,
  onClose,
  onImportComplete,
  properties
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (response.status === 'success' && response.data) {
        ...
        let message = `Successfully imported ${summary.imported} expenses`
        if (summary.skipped > 0) {
          message += `. ${summary.skipped} skipped.`
        }

        showNotification(message, 'success')
        onImportComplete(imported.map(i => i.expense))
        onClose()
      } else {
      ...
      case 'assign':
        return csvData ? (
          <AssignPropertiesStep
            ...
            initialAssignments={propertyAssignments}
            onAssignmentsChange={setPropertyAssignments}
    ...
    <Modal isOpen={isOpen} onClose={onClose} style="w-full max-w-4xl max-h-[90vh]">
      <div className="flex flex-col h-full">
        ...
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-xl font-bold text-gray-900">Bulk Import Expenses</h2>
          <p className="text-sm text-gray-500 mt-1">Import multiple expenses from a CSV file</p>
        </div>
                  ...
                  <div className="flex items-center">
                    ...
                    <span
                      className={`ml-3 text-sm font-medium hidden sm:block ${
                        isActive ? 'text-purple-600' : isComplete ? 'text-green-600' : 'text-gray-500'
                      }`}
                    >
                      {step.label}
                    </span>
...
export default BulkImportExpenseModal
