...
const PROPERTY_FIELDS = [
  ...
  { field: 'commissionRate', label: 'Commission Rate (%)', required: false },
  { field: 'postalCode', label: 'Postal Code', required: false },
  { field: 'externalName', label: 'External Name', required: false },
  { field: 'internalName', label: 'Internal Name', required: false },
...
const MapFieldsStep: React.FC<MapFieldsStepProps> = ({
  ...
  useEffect(() => {
    if (Object.keys(initialMappings).length === 0) {
      const suggestions = suggestPropertyMappings(csvHeaders)
  ...
  const suggestPropertyMappings = (headers: CsvHeader[]): Record<string, string> => {
    const suggestions: Record<string, string> = {}

    const mappingRules = [
      { field: 'listingName', patterns: ['listing', 'name', 'property', 'title'] },
      { field: 'listingId', patterns: ['id', 'listing_id', 'hostaway', 'external'] },
      { field: 'address', patterns: ['address', 'location', 'street'] },
      { field: 'province', patterns: ['province', 'state', 'region'] },
      { field: 'propertyType', patterns: ['type', 'property_type', 'str', 'ltr'] },
      { field: 'commissionRate', patterns: ['commission', 'rate', 'percent'] },
      { field: 'postalCode', patterns: ['postal', 'zip', 'code'] },
      { field: 'externalName', patterns: ['external', 'public'] },
      { field: 'internalName', patterns: ['internal', 'private'] },
      { field: 'description', patterns: ['description', 'desc', 'notes'] }
    ]
  ...
  const validateMappings = (currentMappings: Record<string, string>): boolean => {
    ...
    const requiredFields = PROPERTY_FIELDS.filter(f => f.required)
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="bg-gray-50 px-4 py-3 border-b border-gray-200">
          <div className="grid grid-cols-2 gap-4">
            <span className="text-xs font-semibold text-gray-500 uppercase">Property Field</span>
