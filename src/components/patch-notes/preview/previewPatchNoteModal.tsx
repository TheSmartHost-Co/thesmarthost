    ...
    <Modal isOpen={isOpen} onClose={onClose} style="max-w-2xl w-11/12 p-0 overflow-hidden">
      ...
      <div className="bg-gradient-to-r from-amber-500 to-amber-600 px-6 py-5 text-white">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-white">Preview: {note.title}</h2>
            <p className="text-sm text-amber-100">This is how users will see this note</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-5 max-h-[60vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-2 flex-wrap mb-3">
          ...
          {!note.publishedAt && (
            <span className="bg-gray-100 text-gray-600 text-xs font-medium px-2 py-0.5 rounded-full">Draft</span>
          )}
        ...
        {note.images && note.images.length > 0 && (
          <div className="mt-6">
            <h4 className="text-sm font-medium text-gray-700 mb-3">
              Screenshots ({note.images.length})
            </h4>
