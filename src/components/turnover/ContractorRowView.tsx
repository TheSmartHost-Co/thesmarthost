...
import { layoutTasks, applyStacking, computeStackDepths, getColumnLeft, getColumnWidth } from './utils/calendarEventLayout'
...
interface ContractorRowViewProps {
...
export default function ContractorRowView({
  tasks,
  contractors,
  dateRange,
  onTaskClick,
  ...
  // Skip cancelled tasks entirely in this view
  const visibleTasks = useMemo(() => tasks.filter(tk => tk.status !== 'cancelled'), [tasks])

  const positionedTasks = useMemo(() => {
    const pt = layoutTasks(visibleTasks, allDates)
    applyStacking(pt, item => item.task.contractorId || 'unassigned')
    return pt
  }, [visibleTasks, allDates])

  // Compute max task stack depth per contractor for dynamic row heights
  const maxStackByContractor = useMemo(
    () => computeStackDepths(positionedTasks, item => item.task.contractorId || 'unassigned'),
    [positionedTasks]
  )

  const unassignedCount = useMemo(() => visibleTasks.filter(tk => !tk.contractorId).length, [visibleTasks])
  ...
  const resourceRows: ResourceRow[] = useMemo(() => {
    const rows: ResourceRow[] = [
      ...
      ...contractors.map(c => ({
        ...
        sublabel: c.trade || c.email || c.phone || t('noContact'),
        isUnassigned: false,
        statusBadge: c.status === 'invited' ? t('invited') : c.status === 'inactive' ? t('inactive') : undefined,
      })),
    ]

    // Defensive: tasks can reference a contractor the session-loaded list
    // doesn't know about (assigned from the task modal's own fresh fetch,
    // another tab, etc). Synthesize a row from the task's joined name so the
    // task never silently vanishes from the calendar.
    const knownIds = new Set(rows.map(r => r.id))
    const missing = new Map<string, string>()
    for (const task of visibleTasks) {
      if (task.contractorId && !knownIds.has(task.contractorId) && !missing.has(task.contractorId)) {
        missing.set(task.contractorId, task.contractorName || t('unnamed'))
      }
    }
    for (const [id, name] of missing) {
      rows.push({ id, label: name, sublabel: t('noContact'), isUnassigned: false })
    }
    return rows
  }, [contractors, unassignedCount, visibleTasks, t])

  const tasksByResource = useMemo(() => {
    const map = new Map<string, typeof positionedTasks>()
    for (const pt of positionedTasks) {
      const key = pt.task.contractorId || 'unassigned'
  ...
  // Compute effective row height per contractor
  const getRowHeight = useCallback((contractorId: string) => {
    const stackCount = maxStackByContractor.get(contractorId) ?? 1
    const taskSubRowHeight = stackCount * BAR_H + (stackCount - 1) * STK_GAP
    return Math.max(taskSubRowHeight + ROW_PAD * 2, MIN_ROW_H)
  }, [maxStackByContractor, BAR_H, STK_GAP, ROW_PAD, MIN_ROW_H])
    ...
    <div className="flex overflow-x-hidden overflow-y-visible select-none" style={{ cursor: 'grab', overscrollBehaviorX: 'none' }}>
        ...
        >
          <div className="flex items-center justify-center border-b-2 border-gray-200 bg-gray-50/80 h-10 px-2">
            {isMobile ? (
              ...
              <span className="text-[11px] font-medium text-gray-400 uppercase tracking-wider">{t('contractorsHeader')}</span>
            )}
          </div>
                ...
                {isMobile ? (
                  ...
                  <div className={`flex flex-col justify-center overflow-hidden ${compactDensity ? 'py-1 px-2' : 'py-2 px-3'}`} style={{ height: rowHeight }}>
                    <div className={`font-medium flex items-center gap-1.5 leading-tight ${compactDensity ? 'text-[11px]' : 'text-[13px]'} ${row.isUnassigned ? 'text-amber-700' : 'text-gray-800'}`}>
                      ...
                      {row.statusBadge && (
                        <span className="px-1 py-0.5 text-[10px] bg-gray-100 text-gray-500 rounded capitalize">{row.statusBadge}</span>
                      )}
                    </div>
        ...
        <div ref={timelineRef} className="flex-1 overflow-x-hidden overflow-y-visible relative" style={{ overscrollBehaviorX: 'none' }}>
          ...
          >
            ...
            {/* Resource Rows */}
            {resourceRows.map(row => {
              const rowTasks = tasksByResource.get(row.id) || []
              const rowHeight = getRowHeight(row.id)
              ...
              return (
                ...
                >
                  ...
                  {/* Maintenance task bars with stacking (click-only, not draggable) */}
                  {rowTasks.map(pt => {
                    ...
                    const taskLeft = isMonthView
                      ? colLeft
                      : colLeft + pt.startOffset * colW
                    const taskRight = isMonthView
                      ? colLeft + colW
                      : colLeft + pt.endOffset * colW
                    const taskWidth = Math.max(taskRight - taskLeft, 20)
                    const isExp = allDates[pt.colIndex] === expandedDate
                    const stackTop = ROW_PAD + pt.stackIndex * (BAR_H + STK_GAP)
