...
import { layoutBookings, layoutProjects, getColumnLeft, getColumnWidth } from './utils/calendarEventLayout'
...
export default function PropertyRowView({
  ...
  // When a date is expanded, we need to account for the extra 3× width it takes
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    // Calculate which date column was dropped on using variable widths
        ...
        >
            ...
            return (
              <div className="flex border-b border-gray-100 bg-gray-50/40 h-5 sticky top-10 z-10">
                {allDates.map(dateStr => (
                  ...
                  >
                    {hours.map((hour, i) => {
                      // At 14d zoom (non-24h), only show 6a and 6p (skip midnight and noon)
          ...
          {properties.map(property => {
            ...
            return (
              <div
                key={property.id}
                className="relative border-b border-gray-50 hover:bg-gray-50/20 transition-colors"
                style={{ minHeight: rowMinHeight }}
              ...
              >
                ...
                <div className="absolute inset-0 flex pointer-events-none">
                  {allDates.map((dateStr) => {
                    ...
                    return (
                      <div
                        key={dateStr}
                        className={`flex-shrink-0 border-r ${today ? 'bg-blue-50/30' : headerInfo.isWeekend && isMonthView ? 'bg-gray-50/30' : ''} ${isExpanded ? 'bg-blue-50/20' : ''}`}
                        style={{
                          width: colWidth,
                ...
                {/* Project bars — time-precise positioning, same lane */}
