...
export default function PropertyManagerCleanersPage() {
  ...
  const getCleanerActions = (cleaner: Cleaner): ActionItem[] => {
    ...
    if (canWrite('cleaners')) {
      ...
      actions.push(
        {
          label: t('assignProperties'),
          icon: BuildingOfficeIcon,
          onClick: () => handleAssignProperties(cleaner.id),
          variant: 'default'
        },
        {
          label: t('deleteCleaner', { ns: 'common' }),
          icon: TrashIcon,
    ...
    return (
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t('cleanersTitle')}</h1>
            <p className="text-gray-500 mt-1">{t('cleanersSubtitle')}</p>
          </div>
        </div>
        <div className="flex justify-center items-center h-64">
          <div className="flex flex-col items-center gap-3">
            <div className="w-10 h-10 border-3 border-blue-600 border-t-transparent rounded-full animate-spin"></div>
            <p className="text-sm text-gray-500">{t('loadingCleaners')}</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-2xl p-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-red-800">{t('errorLoadingCleaners')}</h3>
        ...
        <div className="overflow-x-auto overflow-y-auto max-h-[600px]">
          ...
          {filteredCleaners.length === 0 && (
            <div className="text-center py-16 px-4">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-1">{t('noCleanersFound')}</h3>
              <p className="text-gray-500 mb-6 max-w-sm mx-auto">
                {searchTerm || statusFilter !== 'all'
                  ? t('tryAdjustingFilters', { ns: 'common' })
                  : t('getStartedCleaners')}
              </p>
              {!searchTerm && statusFilter === 'all' && (
                <motion.button
                  onClick={() => setShowCreateModal(true)}
