...
const STATUS_FILTER_KEYS: { value: ContractorInvoiceStatus | 'all'; labelKey: string }[] = [
...
const statusConfigBase: Record<ContractorInvoiceStatus, { labelKey: string; bg: string; text: string; dot: string }> = {
...
export default function ContractorInvoicesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [invoicesRes, summaryRes, availableRes] = await Promise.all([
        getContractorInvoices(contractorRes.data.id),
        getInvoiceSummary(contractorRes.data.id),
        getAvailableTasks(contractorRes.data.id),
      ])
  ...
  const handleViewInvoice = (invoice: ContractorInvoice) => {
    setSelectedInvoice(invoice)
    setShowViewModal(true)
  }

  const handleSubmitInvoice = async (invoice: ContractorInvoice) => {
  ...
  const handleDeleteInvoice = async (invoice: ContractorInvoice) => {
    if (!isPM) {
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-5 sm:mb-6">
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.98 }}
          className="inline-flex items-center gap-2 px-4 py-2.5 bg-amber-500 text-white rounded-xl text-sm font-semibold hover:bg-amber-600 shadow-lg shadow-amber-500/25 transition-colors cursor-pointer"
        >
      ...
      {(invoices.length > 0 || (summary && (summary.draft + summary.pending + summary.approved + summary.paid) > 0)) && (
        <div className="flex flex-col items-center gap-2 mb-5 sm:mb-6">
          <motion.button
            ...
            whileTap={{ scale: 0.96 }}
            className="inline-flex items-center gap-3 px-8 py-4 bg-gradient-to-r from-amber-400 to-amber-500 text-white rounded-2xl text-lg font-bold shadow-lg shadow-amber-500/30 hover:shadow-xl hover:shadow-amber-500/40 transition-all cursor-pointer active:shadow-md"
          >
          ...
          {uninvoicedCount > 0 && (
            <p className="text-sm font-medium text-amber-600">
              {t('uninvoicedTasks', { count: uninvoicedCount })}
            </p>
          )}
        </div>
      )}
      ...
      {filteredInvoices.length > 0 ? (
        ...
        >
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="inline-flex items-center gap-2 px-5 py-2.5 bg-amber-500 text-white rounded-xl text-sm font-semibold hover:bg-amber-600 shadow-lg shadow-amber-500/25 transition-colors"
          >
            <PlusIcon className="h-5 w-5" />
