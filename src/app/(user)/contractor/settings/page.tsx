...
import { getContractorByAuthUserId, updateContractor } from '@/services/contractorService'
...
export default function ContractorSettingsPage() {
  ...
  const handleNotificationToggle = async (field: 'smsNotificationsEnabled' | 'emailNotificationsEnabled', enabled: boolean) => {
    ...
    const phone = contractor?.phone || profile.phoneNumber
  ...
  useEffect(() => {
    const fetchContractorData = async () => {
      ...
      try {
        const contractorRes = await getContractorByAuthUserId(profile.id)
        if (contractorRes.status === 'success' && contractorRes.data) {
          setContractor(contractorRes.data)
          // Initialize form with contractor data
          setProfileData({
            fullName: contractorRes.data.name || profile.fullName || '',
            email: contractorRes.data.email || profile.email || '',
            phone: contractorRes.data.phone || '',
            trade: contractorRes.data.trade || '',
          })
          // Initialize invoice settings
          setInvoicePrefix(contractorRes.data.invoicePrefix || '')
          setBusinessName(profile.companyName || '')
          setTaxHstEnabled(contractorRes.data.taxHstEnabled || false)
          setTaxGstEnabled(contractorRes.data.taxGstEnabled || false)
          setTaxQstEnabled(contractorRes.data.taxQstEnabled || false)
        } else {
          // Fallback to profile data if no contractor record
          setProfileData({
            fullName: profile.fullName || '',
            email: profile.email || '',
  ...
  const handleProfileSave = async () => {
    ...
    try {
      ...
      // Also update contractor record if it exists
      if (contractor?.id) {
        const contractorResponse = await updateContractor(contractor.id, {
          name: profileData.fullName,
          phone: profileData.phone || null,
          trade: profileData.trade.trim() || null,
        })

        if (contractorResponse.status === 'success' && contractorResponse.data) {
  ...
  const handleInvoiceSettingsSave = async () => {
    if (!profile?.id || !contractor?.id) return
    ...
    try {
      ...
      // Update invoice prefix and tax defaults on contractor record
      const contractorResponse = await updateContractor(contractor.id, {
          ...
          <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                <div className="w-10 h-10 sm:w-12 sm:h-12 bg-gradient-to-br from-amber-500 to-amber-600 rounded-xl flex items-center justify-center shadow-lg shadow-amber-500/25 flex-shrink-0">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-gray-900 font-medium">
                    {contractor?.name || profile?.fullName || t('notSet')}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
                  ...
                  <p className="text-gray-900 font-medium">
                    {contractor?.email || profile?.email || t('notSet')}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
                  ...
                  <p className="text-gray-900 font-medium">
                    {contractor?.phone || profile?.phoneNumber || t('notSet')}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
                ...
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-gray-900 font-medium">{t('roleContractor')}</p>
                </div>
              </div>
            ) : (
              // Edit Mode
              <div className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent focus:bg-white transition-all"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setProfileData(prev => ({ ...prev, trade: e.target.value }))}
                      className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-transparent focus:bg-white transition-all"
                      placeholder={t('enterTrade')}
                    />
                  </div>
                </div>
                ...
                <div className="flex justify-end gap-3 pt-4 border-t border-gray-100">
                  ...
                  <motion.button
                    ...
                    whileHover={{ scale: 1.02 }}
                    whileTap={{ scale: 0.98 }}
                    className="px-5 py-2.5 text-white bg-amber-600 rounded-xl font-medium hover:bg-amber-700 shadow-lg shadow-amber-500/25 transition-colors disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                    disabled={saving}
                  >
          ...
          <div className="px-4 sm:px-6 py-4 sm:py-5 border-b border-gray-100">
            <div className="flex items-center gap-3 sm:gap-4">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs sm:text-sm text-gray-500">{t('notificationsDescContractor')}</p>
              </div>
            </div>
          </div>
              ...
              <div className="flex items-start sm:items-center justify-between gap-3 p-4 bg-gray-50 rounded-xl">
                ...
                <button
                  ...
                  className={`relative inline-flex h-7 w-12 sm:h-6 sm:w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 mt-0.5 sm:mt-0 ${
                    (profile?.emailNotificationsEnabled ?? true) ? 'bg-amber-600' : 'bg-gray-200'
                ...
                <div className="flex items-start sm:items-center gap-3 sm:gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs sm:text-sm text-gray-500 mt-0.5">
                      {t('smsAlertsForTasks')}
                      {!(contractor?.phone || profile?.phoneNumber) && (
                ...
                <motion.button
                  onClick={() => {
                    if (showInvoiceEdit) {
                      ...
                      setInvoicePrefix(contractor.invoicePrefix || '')
                      setBusinessName(profile?.companyName || '')
              ...
              {!showInvoiceEdit ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
                  ...
                  <div className="bg-gray-50 rounded-xl p-4">
                    ...
                    <p className="text-gray-900 font-medium">
                      {contractor.invoicePrefix || t('autoFromInitials')}
                    </p>
                  </div>
                  <div className="bg-gray-50 rounded-xl p-4 md:col-span-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {([
                        { key: 'hst' as const, enabled: contractor.taxHstEnabled },
                        { key: 'gst' as const, enabled: contractor.taxGstEnabled },
                        { key: 'qst' as const, enabled: contractor.taxQstEnabled },
                      ]).map(({ key, enabled }) => (
                        <span
                          key={key}
            ...
            <div className="p-4 sm:p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-6">
                <div className="bg-gray-50 rounded-xl p-4">
                  ...
                  <p className="text-gray-900 font-medium">
                    {contractor.hourlyRate ? `$${contractor.hourlyRate.toFixed(2)}/hr` : t('notSet')}
                  </p>
                </div>
                <div className="bg-gray-50 rounded-xl p-4">
                  <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">
                  ...
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold ${
                    ...
                    {contractor.status === 'active' ? t('active') : contractor.status === 'invited' ? t('invited') : t('inactive')}
                  </span>
                </div>
              </div>
